...
export class ChatbotComponent implements AfterViewChecked {
  ...
  ask(text?: string) {
    ...
    this.http.post<any>('/api/chatbot/ask', { question: q }).subscribe({
      next: (res) => {
  ...
  summarizeDB() {
    ...
    this.http.get<any>('/api/chatbot/summarize').subscribe({
      next: (res) => {
        const idx = this.messages.indexOf(loadingMsg);
  ...
  onFileSelected(event: Event) {
    ...
    this.http.post<any>('/api/chatbot/upload', formData).subscribe({
      next: (res) => {
        const idx = this.messages.indexOf(loadingMsg);
