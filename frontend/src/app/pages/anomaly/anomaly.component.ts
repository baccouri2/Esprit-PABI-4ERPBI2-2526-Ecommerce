...
export class AnomalyComponent {
  ...
  detectAnomalies() {
    // Input validation
    if (this.sensitivity < 0.01 || this.sensitivity > 0.5) {
      this.error = 'Sensitivity must be between 0.01 and 0.5';
      return;
    }
    this.loading = true;
    this.error = '';
    this.api.detectAnomalies(this.selectedModel, this.sensitivity).subscribe({
      next: (res) => {
