...
Fixed version with lazy statsmodels loading
"""
...
# Lazy load statsmodels only when needed
...
def _ensure_statsmodels():
    """Lazy load statsmodels to avoid startup hang"""
    global _statsmodels_loaded, ARIMA, SARIMAX, ExponentialSmoothing
    if not _statsmodels_loaded:
        try:
            ...
            print(f"Warning: Could not load statsmodels: {e}")
            raise
